...
open-Xdownload 是一个本地优先的 X / Twitter 媒体下载器。内置 Web 界面与后台任务队列，支持单条推文、用户、列表、关注四种任务类型，提供批量任务、定时计划、全量归档、失败重试、Cookie 池与本地目录存储。
...
- 一键补齐视频封面：媒体库支持手动触发批量补齐缺失的视频预览图，运行中实时显示进度，已有封面的记录自动跳过。
